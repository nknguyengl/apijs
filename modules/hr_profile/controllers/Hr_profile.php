  ...
  redirect(admin_url('hr_profile/staff_infor'));
	...
	public function member($id = '', $group = '')
	{
		...
		if ($id == '') {
			...
			if(get_staff_user_id() != $id && !is_admin() && !has_permission('hrm_hr_records', '', 'edit') && !has_permission('hrm_hr_records', '', 'view')){
				access_denied('staff');
			}
...
public function update_staff_profile(){
  ...
  if ($data['id'] == '') {
	...
	public function resignation_procedures(){
		$this->app_scripts->add('circle-progress-js','assets/plugins/jquery-circle-progress/circle-progress.min.js');
		if(!has_permission('hrm_procedures_for_quitting_work', '', 'view') && !is_admin() ){   
	...
	{
		if (!has_permission('hrm_hr_records', '', 'view') && get_staff_user_id() != $id) {
			access_denied('staff');
		}
